...
**Only once `CONTEXT.md` exists.** Until then this section is dormant: use the
vocabulary `CLAUDE.md` already establishes (`static export`, `web root`,
`deploy workflow`, `lint baseline`) and don't treat an undocumented term as a
domain gap — with no glossary, every term is undocumented, and a rule that
fires on everything is a rule that says nothing.

Once it exists: when your output names a domain concept (in an issue title, a
refactor proposal, a hypothesis, a test name), use the term as defined in
